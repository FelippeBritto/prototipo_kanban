  ...
  <v-layout>
    <v-row ref="table" fill-height>
      ...
      >
        ...
        <v-btn fab absolute dark color="green" class="card-add-button" @click="newTask(column.id)">
    ...
    <v-dialog v-model="editTaskOpened" width="500">
      <v-card>
        <v-toolbar
          ...
          dark
          class="mb-6"
        >{{ taskInstance.id ? 'Editar Processo' : 'Criar Processo' }}</v-toolbar>
        <v-card-text>
          <v-text-field v-model="taskInstance.title" label="Material" />
          <v-text-field v-model="taskInstance.text" label="Código" />
        </v-card-text>
        <v-card-actions>
          <v-spacer />
          <v-btn text @click="editTaskOpened = false">Cancelar</v-btn>
          <v-btn text @click="saveTask">Salvar</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <v-dialog v-model="unlockRequestOpened" width="400">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn text @click="unlockRequestOpened = false">Cancelar</v-btn>
          <v-btn
            text
            v-if="!unlockRequestTarget.locking_requested"
            @click="sendUnlockRequest"
          >Send Unlock Request</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
export default {
  ...
  computed: {
    maxDt() {
      return this.$store.getters["kanban/maxDt"];
    },
    tasks() {
      return this.$store.state.kanban.tasks;
    },
    unlockRequestTarget() {
      return (
        this.tasks.find(task => task.id === this.unlockRequestTargetId) || {}
      );
    },
    columnsTrigger() {
      return JSON.stringify(
        this.tasks.map(task => _.pick(task, "id", "col", "sort", "updated_at"))
      );
    },
    editPermissionRequested() {
      const task = this.tasks.find(task => task.edit_permission);
      return task && task.edit_permission_requested_by_someone_else
        ? task.id
        : null;
    }
  },
  watch: {
    ...
    unlockRequestOpened(value) {
      if (
        this.unlockRequestTarget.id &&
        this.unlockRequestTarget.locking_requested &&
        !value
      ) {
  ...
  methods: {
    runUpdateQueue() {
      this._timeout && clearTimeout(this._timeout);
      this.$store.dispatch("kanban/loadTasks").then(() => {
        this._timeout = setTimeout(this.runUpdateQueue, 60000);
      });
    },
    buildColumns() {
      const columns = new Array(this.columnsCount)
        .fill(null)
        .map((value, id) => ({ id, tasks: [] }));
      ...
      axios
        .request({
          ...
          data: {
            ...
            beforeId: newIndex
              ? this.columns[newColumn].tasks[newIndex - 1].id
              : null
          }
        })
        .catch(error => {
    ...
    saveTask() {
      ...
      axios
        .request({
          ...
          url: this.taskInstance.id
            ? `/tasks/${this.taskInstance.id}`
            : "/tasks",
        ...
        .then(() => {
          this.$snack({
            type: "success",
            text: this.taskInstance.id ? "Task Updated" : "New Task Created"
          });
    ...
    checkIfUnlockAnswerReceived() {
      if (!this.unlockRequestTarget.id) {
        return false;
      }
